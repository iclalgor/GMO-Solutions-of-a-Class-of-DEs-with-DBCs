...
% GMO: Geometric Mean Optimizer                                                                 
function [z_iter,z_final,pos_final] = GMO(problem, params)
model = problem.model;
x = problem.x;
a = problem.a;
b = problem.b;
...
mutant=zeros(1,nx);
stdev2=zeros(1,nx);
index=zeros(1,np);%uygunluklarına göre sıralanınca indekslerini tutmak için
fit=zeros(maxit,np);
DFI=zeros(1,np);
optimal_pos=zeros(1,nx);
z_pbest=zeros(np,1) ;% her ajan için elde edilen en düşük uygunluk değeri
%z_kbest=zeros(np,1);
z_optimal=inf*ones(maxit,1);
pos_final=zeros(1,nx);
z_iter=zeros(maxit,1);%her it'daki en düşük uygunluk değeri
kbest_max=np; %elit çözüm sayısı
kbest_min=2;
...
kbest=kbest_max-(kbest_max-kbest_min)*(it/maxit); %elit sayısı
n_best=round(kbest); %yuvarlama
z_kbest=zeros(n_best,1);
...
%     disp(['Number of Iterations = ',num2str(it)]);
    for j=1:np 
        ...
        %hızları sınırlar içinde tutmak için
    ...
    %standart sapmaları hesaplar en büyüğünü bulur
    for i=1:nx
        stdev2(1,i)=std(pp_pbest(1:np,i));
    end
    max_stdev2=max(stdev2(1,1:nx));

    %Şimdiye kadar bulunan en iyi çözümlerin ss ve ort.
    %  Calculating Mean and Std of the best-so-far solutions' objective values
    ave=mean(z_pbest(1:np));
    stdev=std(z_pbest(1:np));
    
    %Elit birey sayısını belirleme
    % Determining the number of the Elite best-so-far solutions
    kbest=kbest_max-(kbest_max-kbest_min)*(it/maxit);
    n_best=round(kbest);
    z_kbest=zeros(n_best,1);

    %ikili uygunluk indeksini hesaplama
    % Evaluating the Dual-Fitness Indices of the solutions- Eq.(3)
    for j=1:np
        index(j) = j;
    end
...
%     end
    ...
    % Saving the best-so-far objective value in the current run
    z_iter(it)=z_optimal(it);
    fprintf('Best cost at iteration %d is %8.4e\n',it,z_iter(it));
end
